...
3. `query()` options: `tools: ["Skill"]` whitelists only the built-in `Skill` tool so the agent can invoke the knowledge skills under `.claude/skills/` (`gsap-core`/`-timeline`/`-utils`/`-plugins`/`-react`/`-frameworks`/`-performance`/`-scrolltrigger` via symlinks into `vendor/gsap-skills/`, plus `disney-12-principles` and `design-atlas`); all other built-in tools (Bash/Read/Write/Edit/…) stay disabled, so scene mutation still only happens through our MCP tools and skills are read-only knowledge. `permissionMode: "bypassPermissions"` + `allowDangerouslySkipPermissions: true` skips prompts. The CLI auto-injects the skill list (name+description) from `.claude/skills/`, and `prompts.ts` points the agent at which skill to reach for when (e.g. call `gsap-plugins` when unsure about a plugin API). Without this entry the agent is tool-blind to skills — it would mention `gsap-plugins` in thinking then give up ("Actually, let me…"), which is exactly the bug this line fixes.
